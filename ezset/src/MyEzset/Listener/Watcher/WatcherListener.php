...
namespace MyEzset\Listener\Watcher;
 ...
 */
class WatcherListener extends \JEvent
{
	/**
	...
	public function onAfterRoute()
	{
		...
		if ($app->isSite())
		{
			$easyset = \JPluginHelper::getPlugin('system', 'ezset');

			$params = new \JRegistry(json_decode($easyset->params));

			$app->redirect(\JUri::root() . 'administrator/index.php?option=com_watcher&' . $params->get('adminSecureCode', ''));
	...
	public function onAfterDispatch()
	{
		...
		if ($app->isAdmin())
		{
			...
			if ($option == 'com_cpanel' && $view != 'login')
			{
				$app->redirect('index.php?option=com_watcher');
